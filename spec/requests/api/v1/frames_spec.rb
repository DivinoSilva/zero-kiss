...
RSpec.describe "Frames API", type: :request do
  ...
  let(:headers) { auth_headers } # vem de spec/support/auth_helpers.rb
  ...
  describe "POST /api/v1/frames" do
    ...
    context "when payload is valid" do
      it "creates and returns 201" do
        post path, params: { frame: { center_x: 10, center_y: 10, width: 20, height: 30 } }, headers:
    ...
    context "when payload is invalid" do
      it "returns 422 with errors" do
        post path, params: { frame: { center_x: 0, center_y: 0, width: 0, height: -1 } }, headers:
    ...
    context "frame separation rule - edge touch" do
      it "returns 422" do
        ...
        post path, params: { frame: { center_x: 10, center_y: 0, width: 10, height: 10 } }, headers:
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
    ...
    context "frame separation rule - corner touch" do
      it "returns 422" do
        create(:frame, center_x: 0, center_y: 0, width: 10, height: 10)
        post path, params: { frame: { center_x: 10, center_y: 10, width: 10, height: 10 } }, headers:
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
    ...
    context "frame separation rule - overlap" do
      it "returns 422" do
        create(:frame, center_x: 0, center_y: 0, width: 10, height: 10)
        post path, params: { frame: { center_x: 4, center_y: 0, width: 10, height: 10 } }, headers:
        expect(response).to have_http_status(:unprocessable_entity)
      end
    end
    ...
    context "when there is minimum separation (epsilon = 0.001)" do
      it "returns 201" do
        create(:frame, center_x: 0, center_y: 0, width: 10, height: 10)
        post path, params: { frame: { center_x: 10.001, center_y: 0, width: 10, height: 10 } }, headers:
        expect(response).to have_http_status(:created)
      end
    end
    ...
    context "strict separation" do
      it "returns 201" do
        create(:frame, center_x: 0, center_y: 0, width: 10, height: 10)
        post path, params: { frame: { center_x: 11, center_y: 0, width: 10, height: 10 } }, headers:
        expect(response).to have_http_status(:created)
      end
    end
  end
  ...
  describe "GET /api/v1/frames/:id" do
    context "when the frame exists" do
      ...
      it "returns 200 with payload and extremal circles" do
        ...
        get "/api/v1/frames/#{frame.id}", headers:
    ...
    context "when the frame does not exist" do
      it "returns 404 with error" do
        get "/api/v1/frames/999_999", headers:
  ...
  describe "DELETE /api/v1/frames/:id" do
    context "when the frame has circles" do
      it "returns 422 with errors" do
        ...
        delete "/api/v1/frames/#{frame.id}", headers:
        expect(response).to have_http_status(:unprocessable_entity)
        expect(json).to have_key("errors")
      end
    end
    ...
    context "when the frame does not exist" do
      it "returns 404" do
        delete "/api/v1/frames/999_999", headers:
        expect(response).to have_http_status(:not_found)
      end
    end
  end
