...
RSpec.describe "Circles API", type: :request do
  ...
  describe "POST /api/v1/frames/:frame_id/circles" do
    it "creates (201) when it fits and does not touch others" do
      post "/api/v1/frames/#{frame.id}/circles",
           params: { circle: { center_x: 0, center_y: 0, diameter: 6 } }, headers: headers
    ...
    it "returns 422 when the circle would touch another" do
      ...
      post "/api/v1/frames/#{frame.id}/circles",
           params: { circle: { center_x: 3, center_y: 0, diameter: 6 } }, headers: headers
    ...
    it "returns 422 when the circle exceeds the frame bounds" do
      post "/api/v1/frames/#{frame.id}/circles",
           params: { circle: { center_x: 20.1, center_y: 0, diameter: 40 } }, headers: headers
      expect(response).to have_http_status(:unprocessable_entity)
      expect(json).to have_key("errors")
    end
  ...
  describe "PUT /api/v1/circles/:id" do
    it "updates and returns 200" do
      ...
      put "/api/v1/circles/#{circle.id}", params: { circle: { center_x: 1 } }, headers: headers
    ...
    it "returns 422 if update would move circle outside the frame" do
      ...
      put "/api/v1/circles/#{circle.id}", params: { circle: { center_x: 10.1 } }, headers: headers
      expect(response).to have_http_status(:unprocessable_entity)
      expect(json).to have_key("errors")
    end
    ...
    it "returns 422 if update would make circles touch/overlap" do
      ...
      put "/api/v1/circles/#{b.id}", params: { circle: { center_x: 3 } }, headers: headers
      expect(response).to have_http_status(:unprocessable_entity)
      expect(json).to have_key("errors")
    end

    it "returns 404 for unknown id" do
      put "/api/v1/circles/999_999", params: { circle: { center_x: 1 } }, headers: headers
      expect(response).to have_http_status(:not_found)
    end
  end
  ...
  describe "DELETE /api/v1/circles/:id" do
    ...
    it "returns 404 for unknown id" do
      delete "/api/v1/circles/999_999", headers: headers
      expect(response).to have_http_status(:not_found)
    end
  end
  ...
  describe "GET /api/v1/circles (search + filter + pagination)" do
    ...
    it "filters by frame_id" do
      get "/api/v1/circles", params: { frame_id: frame.id }, headers: headers
      expect(response).to have_http_status(:ok)
    ...
    it "radius query returns only circles strictly inside (r - d/2)" do
      get "/api/v1/circles", params: { center_x: 0, center_y: 0, radius: 3 }, headers: headers
      expect(response).to have_http_status(:ok)
      ids = json.map { |h| h["id"] }
    ...
    it "returns empty when radius < diameter/2" do
      get "/api/v1/circles", params: { center_x: 0, center_y: 0, radius: 0.5 }, headers: headers
      expect(response).to have_http_status(:ok)
      expect(json).to eq([])
    end

    it "respects per_page parameter" do
      get "/api/v1/circles", params: { per_page: 1, page: 1 }, headers: headers
      expect(response).to have_http_status(:ok)
    ...
    it "clamps per_page to the maximum (200)" do
      ...
      get "/api/v1/circles", params: { per_page: 1_000, page: 1 }, headers: headers
      expect(response).to have_http_status(:ok)
